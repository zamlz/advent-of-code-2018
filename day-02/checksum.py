...
with open(sys.argv[1], 'r') as f:
    boxids = [ x[:-1] for x in f.readlines() ]
...
for bid in tqdm(boxids):
    counts = collections.Counter(bid).values()
    if 2 in counts:
        c2 += 1
    if 3 in counts:
        c3 += 1
...
for i in range(len(boxids)):
    for j in range(i+1, len(boxids)):
        x = boxids[i]
        y = boxids[j]
        for k in range(len(boxids[i])):
            if x[:k]+x[k+1:] == y[:k]+y[k+1:] :
                print("Common Letter BoxID: {}".format(x[:k]+x[k+1:]), end='')
                print(" = '", x[:k], "' + <", x[k], end='')
                print(", ", y[k], "> + '", x[k+1:], "'")
                exit()
