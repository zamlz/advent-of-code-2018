...
def reduction(poly):
    ...
    while i < (len(poly) - 1):
        ...
        if ((poly[i].upper() == poly[i+1] and poly[i] == poly[i+1].lower()) or
            (poly[i].lower() == poly[i+1] and poly[i] == poly[i+1].upper())):
...
print("\nOriginal Unit Count: ", len(polymer))
print("Units remaining: ",       len(reduction(polymer)))
...
def remove_all_units(poly, unit):
    poly = poly.replace(unit.lower(), '')
    poly = poly.replace(unit.upper(), '')
    return poly

polylist = []
for c in ascii_lowercase:
    poly = remove_all_units(polymer, c)
    polylist.append(len(reduction(poly)))

print("Best possible reaction length: ", min(polylist))
